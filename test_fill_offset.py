"""Test script for fill generation with centerline offsetting enabled."""
...
def main():
    """Test the fill generator with centerline offsetting enabled."""
    ...
    # Generate fill paths WITH offsetting enabled
    print("\nGenerating fill paths (WITH centerline offsetting)...")
    fill_gen = FillGenerator(line_spacing=0.1)
    paths = fill_gen.generate_fill(geometry, trace_centerlines=trace_centerlines, offset_centerlines=True)
    ...
    output_file = Path("output/fill_visualization_offset.png")
    ...
    viz.plot_paths(paths, color="cyan", alpha=0.8, linewidth=0.5, label="Fill paths (with offset)")

    viz.set_bounds(*bounds, margin=2.0)
    viz.add_labels(title="PCB Fill Pattern (with centerline offsetting)")
